function fetchPatientData() {
    fetch('/api/method/library_management.library_management.doctype.patient_visit.patient_visit.get_patient_data7')
        .then(response => response.json())
        .then(r => {
            if (r.message && r.message.length > 0) {
                displayPatientData(r.message);
            } else {
                document.getElementById("result").innerHTML = "<p>No patient visit data found.</p>";
            }
        })
        .catch(err => {
